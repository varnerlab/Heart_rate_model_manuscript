...
\section*{Discussion}
We estimated three sets of parameters that can be used to predict heart rates of patients in intensive care units. The accuracy of the model was much improved when patients were grouped by their characterstics. 

The model used has several shortcommings. This model oversimplifies the process of heart rate regulation. It does not include cardiac sypathetic afferrent reflex, which is sensitive to both mechanical and chemical inputs. \cite{du2007love} It can only be used for short time scales, as it does not include the slower acting renin-agiotensin-aldostrone system, which determines blood volume, and therefore blood pressure. \cite{boron2016medical} Nevertheless, it does captures the dynamics of the system at short time scales.
\section*{Methods}
...
Since pressure data was given in a discrete format, we linearly interpolated between values when neccessary, as pressure is an input to the system of differential equations.
\bibliography{draft1}
...
\begin{figure}[ht]
       ...
       \caption{The black marks are from cluster 1, the grey markss represent cluster 2. Error bars are omitted for the sake of clarity. Parameters represented by an square were allowed to vary, and parameters represented by a diamond were held constant.}
